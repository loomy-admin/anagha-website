...
import { isInWishlist, toggleWishlist, removeFromWishlist, WISHLIST_CHANGED_EVENT, type WishlistItem } from '@/lib/wishlist';
...
export default function WishlistProductCard({ product }: { product: CatalogItem }) {
  ...
  return (
    <div className="group flex flex-col bg-white rounded-lg overflow-hidden border border-gray-100 hover:shadow-lg transition-shadow duration-300 h-full relative">
      {/* Heart Button */}
      <button
      ...
      {/* Main Link Area */}
      <Link href={itemHref(product)} className="flex flex-col flex-1">
        <div className="relative w-full aspect-[5/4] bg-[#fafafa] overflow-hidden shrink-0">
        ...
        <div className="p-3 flex flex-col flex-1 items-start text-left bg-white">
          ...
          <h3 className="text-[#666] text-[12px] md:text-[13px] line-clamp-2 leading-snug min-h-[32px] w-full">
            {product.name}
          </h3>
      ...
      {/* Action Buttons specific to Wishlist */}
      <div className="px-2 md:px-3 pb-3 bg-white flex flex-col lg:flex-row items-center gap-1.5 mt-auto pt-1 w-full">
        <div className="w-full lg:flex-1 min-w-0">
          <AddToCartButton 
            item={product as any} 
            className="!py-1.5 !px-2 w-full !text-[10px] md:!text-[11px] !tracking-normal !rounded-full"
            onAfterAdd={() => removeFromWishlist(product.tag_number)} 
          />
        </div>
        <div className="w-full lg:flex-1 min-w-0">
          <BuyNowButton 
